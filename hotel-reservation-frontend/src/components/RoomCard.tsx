...
// version: 3.6.0
// FEATURE: Show "Select Date" prompt when no pricing/dates are available instead of booking controls.

import React, { useState, useEffect, useMemo } from 'react';
import Image from 'next/image';
import { AvailableRoom, CartItem, PricedBoardType } from '@/types/hotel';
import { Users, BedDouble, UserPlus, Baby, PlusCircle, CheckCircle, Loader2, CalendarDays, X, CalendarSearch } from 'lucide-react';
...
const RoomCard: React.FC<RoomCardProps> = ({ room, duration, onAddToCart, reservedCount, hotelId }) => {
  ...
  // بررسی اینکه آیا اطلاعات قیمت‌گذاری (و در نتیجه تاریخ) وجود دارد یا خیر
  const hasPricing = room.priced_board_types && room.priced_board_types.length > 0;
  ...
  }, [room.priced_board_types, hasPricing]);
  ...
  return (
    <>
        ...
        {/* Column 1: Image */}
        <div className="w-full md:w-1/5 flex-shrink-0 relative min-h-[200px] md:min-h-0 bg-gray-100">
            <Image 
                src={roomImage} 
                alt={room.name}
                fill
        ...
        <div className="w-full md:w-1/2 p-4 flex flex-col justify-between">
            ...
            <p className="text-sm text-gray-500 mt-3 border-t pt-3 line-clamp-2 leading-relaxed">{room.description}</p>
            </div>
            
            {/* Controls Section - ONLY Show if Pricing Exists */}
            {hasPricing ? (
                <div className="mt-4 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3 p-3 bg-gray-50 rounded-lg border border-gray-100 items-center">
                        {/* Guest Selectors... (Same as before) */}
                        {maxExtraAdults > 0 && (
                            <div className="flex items-center gap-2">
                                ...
                                <select 
                                    ...
                                    className="text-sm border rounded px-1 py-0.5 bg-white outline-none focus:border-blue-500 cursor-pointer"
                                >
                        ...
                        <div className="mr-auto">
                            ...
                            >
                                ...
                                <span className="group-hover:underline decoration-blue-300 underline-offset-4">تقویم قیمتی</span>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-4 p-4 bg-orange-50 border border-orange-100 rounded-lg flex items-start gap-3">
                    <CalendarSearch className="w-5 h-5 text-orange-500 flex-shrink-0 mt-0.5" />
                    <div>
                        <p className="text-sm font-bold text-orange-800">بررسی قیمت و موجودی</p>
                        <p className="text-xs text-orange-700 mt-1 leading-relaxed">
                            برای مشاهده قیمت دقیق و رزرو این اتاق، لطفا تاریخ ورود و خروج خود را از بخش جستجو انتخاب کنید.
                        </p>
                    </div>
                </div>
            )}
            ...
            {hasPricing ? (
                <>
                    ...
                    <div className="my-2">
                        ...
                        {finalPricePerRoom > 0 ? formatPrice(finalPricePerRoom) : 'رایگان'}
                        {finalPricePerRoom > 0 && <span className="text-sm font-normal text-gray-600 mr-1">ریال</span>}
                        </p>
                        {quantity > 1 && (
                    ...
                    {(extraAdults > 0 || childrenCount > 0) && (
                        <p className="text-[10px] text-green-600 bg-green-50 px-2 py-0.5 rounded-full mb-2 border border-green-100">
                            (شامل هزینه نفرات اضافه)
                        </p>
                    )}
                    
                    {maxAvailableQuantity <= 0 ? (
                        <p className="text-sm text-red-500 font-bold mt-2 bg-red-50 px-3 py-1 rounded border border-red-100">ظرفیت تکمیل است</p>
                    ) : (
                        <div className="w-full mt-4 space-y-3">
                            <div className="flex items-center justify-center gap-2 text-sm">
                                ...
                                <select 
                                    ...
                                    className="border rounded px-2 py-1 bg-white outline-none focus:border-blue-500 cursor-pointer"
                                >
                ...
                // --- Placeholder for No Pricing ---
                <div className="flex flex-col items-center justify-center h-full text-gray-400">
                    <CalendarSearch className="w-10 h-10 mb-2 opacity-50" />
                    <span className="text-sm font-medium">---</span>
                    <span className="text-xs mt-1">قیمت نامشخص</span>
                </div>
            )}
        </div>
        </div>
                ...
                >
                    <div className="p-4 border-b flex justify-between items-center bg-gray-50">
                        <h3 className="font-bold text-gray-800 flex items-center gap-2">
                             ...
                             تقویم قیمتی: <span className="text-blue-700">{room.name}</span>
                        </h3>
                        <button 
                            onClick={() => setIsCalendarOpen(false)}
